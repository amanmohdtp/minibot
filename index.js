    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
        ...
        if (cmd === ".kick" && isGroup) {
            ...
            if (!target) {
                ...
                if (m.message.extendedTextMessage?.contextInfo?.mentionedJid?.length > 0) {
                    ...
                    return sock.sendMessage(from, { text: "Mention a user to kick or reply to their message." });
                }
            }
        ...
        if (cmd === ".warn" && isGroup) {
            ...
            // Check for mentions
                ...
                target = m.message.extendedTextMessage.contextInfo.mentionedJid[0]; // first mention
            }

            if (!target) return sock.sendMessage(from, { text: "Mention a user or reply to their message." });
...
start();        if (antiLink && isGroup && linkPattern.test(text) && !isAdmin && !isOwner) {
            try { await sock.sendMessage(from, { delete: m.key }); } catch {}
